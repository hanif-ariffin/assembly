...
eal_while2:                     ; while(!delayFlag) 
   ...
   bne eal_endwhile2
   jsr pollDelay                ;   delayFlag = pollDelay(); 
   staa EAL_DELAYFLAG,SP
   jsr pollgetchar              ;   input = pollgetchar();  // check if code entered to disarm
   stab EAL_INPUT,SP
   tba
eal_if1:
...
eal_while3                      ;  while(!codeValid)
   tst EAL_CODEVALID,SP         ; {
   bne eal_endwhile3
   jsr pollgetchar              ;    input = pollgetchar();b
   stab EAL_INPUT,SP
   tba
eal_elseif
   cmpa #'#'                    ;    if(isdigit(input) || input == '#') 
...
eal_blk2:      
   ...
   bra eal_endelseif    
eal_elseifA:
   ldaa EAL_INPUT,SP             ;   else if(input == 'a') // Front door opened
   cmpa #'a'
   bne eal_elseifB               ;   {
...
eal_if4:
   ...
   bra eal_endif4                ;         }
eal_else4                        ;         else {
   jsr pollgetchar               ;            input = pollgetchar();  // check if alarm code entered to disarm
   stab EAL_INPUT,SP
eal_if5:
...
eal_endelseifA                  ;      }
   bra eal_endelseif;      
eal_elseifB:
   ldaa EAL_INPUT,SP            ;      else if(input >='b' && input <= 'e') // other door/window opened
   cmpa #'b'                    ;      {
   blo eal_endelseifB
   cmpa #'e'
   bhi eal_endelseifB
...
isCodeValid: pshx
   ...
   ; int *ptr;  // pointer to alarmCodes
   movb #NUMCODES,CDV_CNT,SP ; byte cnt = NUMCODES;
...
tra_while2:                  ;    while(!doneInput) 
   ...
   bne tra_endwhile2
   jsr pollDelay             ;       doneInput = pollDelay();
